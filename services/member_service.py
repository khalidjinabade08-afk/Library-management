...
def create_member(data):
    try:
        existing_member = Member.query.filter_by(name = data["name"], phone_no = data["phone_no"]).first()
...
def search_membership(page=1, per_page=4,**kwargs):
    try:
        ...
        if kwargs.get("membership_type"):
            membership = membership.where(Membership.membership_type == kwargs["membership_type"])
...
def delete_member(member_id):
    try:
        ...
        return success_response("deleted succefully",{
            "deleted member id":member_id
        })
