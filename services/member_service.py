...
def add_membership(data):
    try:
        ...
        if membership_type == "1 week":
            ...
            return error_response("Invalid membership type")
...
def show_membership(page=1, per_page=4):
    try:
        ...
        for membership in page_obj.items:
            status = "expired" if membership.end_date < date.today() else "active"
...
def search_membership(membership_type=None, page=1, per_page=4):
    try:
        query = select(Membership)

        if membership_type:
            query = query.where(Membership.membership_type == membership_type)

        query = query.order_by(Membership.id.desc())

        # Get total matching records for proper pagination
        total_matching = len(db.session.execute(query).scalars().all())
...
def search_member(**kwargs):
    ...
    if kwargs.get("id"):
        query = query.where(Member.id == kwargs["id"])

    if kwargs.get("name"):
        query = query.where(Member.name.like(f"%{kwargs['name']}%"))
...
def delete_member(member_id):
    try:
        ...
        return success_response("deleted successfully", {"deleted member id": member_id})
